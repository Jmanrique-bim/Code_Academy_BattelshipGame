...
# Transform letters from input to numbers base on a dictionary



# Print the board interface in the terminal
    ...
    for ship_length in LENGTH_OF_SHIPS:
        while True:
            if board == COMPUTER_BOARD:
                ...
                if check_ship_fit(ship_length,orientation,row,column):
                ...
                orientation, row, column = user_input(place_ship)
                if check_ship_fit(ship_length,orientation,row,column):
                    #Check if ship overlaps
                    if ship_overlaps(board, row, column, orientation, ship_length)  == False:
                        #Place ship
...
def check_ship_fit(ship_length ,orientation, row, column):
    if orientation == "H":
